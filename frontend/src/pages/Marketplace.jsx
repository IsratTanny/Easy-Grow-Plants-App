...
import { staticAsset, isNative, getMediaBase } from '../utils/platform';
...
const img = (file) => staticAsset(`images/${file}`);
...
export default function Marketplace() {
    ...
    const getImagePath = (plant) => {
        ...
        // On a native build relative paths can't be proxied — resolve against
        // the backend origin so /media and /static assets load on the phone.
        if (isNative()) {
            const p = url.startsWith('/media/') || url.startsWith('/static/') || url.startsWith('/images/')
                ? url
                : `/media/${url.replace(/^\//, '')}`;
            return `${getMediaBase()}${p}?v=1779038000`;
        }
        let path = "";
